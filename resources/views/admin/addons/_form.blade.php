{{-- 
    ...
    Variabel $addon akan ada jika ini mode 'edit', dan tidak ada jika mode 'create'
--}}

@csrf

<div class="mb-4">
    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nama Addons</label>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
    <div>
        ...
        <input type="number" name="price" id="price" 
               ...
               value="{{ old('price', $addon->price ?? '') }}" required min="0">
        @error('price')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
...
<div class="mb-6">
    <label class="flex items-center">
        <input type="checkbox" name="is_active" value="1"
               ...
               @checked(old('is_active', $addon->is_active ?? false)) 
        >
        <span class="ml-2 text-sm text-gray-700">Aktifkan Addons</span>
    </label>
</div>
